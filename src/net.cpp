...
static const char *strDNSSeed[][2] = {
    {"adamjcasey.com", "dnsseed.adamjcasey.com"},
    {"ryanvarley.uk", "dnsseed.ryanvarley.uk"},
};
...
void ThreadOpenAddedConnections2(void* parg)
{
    ...
    if (mapArgs.count("-addnode") == 0)
        mapMultiArgs["-addnode"].push_back("50.179.191.36");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. nlocoin is probably already running."), addrBind.ToString().c_str());
        else
